...
ENTRY_REVERT_SECS = 4.5  # [^] window after an entry in which a SELLER flip cancels it
...
# 3 areas of the shop. When an INTERACTION is counted, it is attributed to whichever
# zone the SERVED CLIENT is standing in at that moment (their box center); the
# per-zone counts sum to the total. An interaction that happens outside all 3 zones
# still counts in the total but is not attributed to a zone (so the zone sum can be
# < total -- draw the zones to cover every serving area if you want sum == total).
...
PEC_ZONE_2 = [
    [(1909, 380), (1681, 538), (1558, 795), (1909, 723), (1909, 379)]   # <-- REDRAW (middle area)
]
PEC_ZONE_3 = [
...
def pec_zone_of_point(x: float, y: float) -> int:
    ...
    Used to attribute a counted interaction to the area the served client stands in."""
...
def draw_frame(frame, persons, entry_count, interaction_count=0, served=None,
               persons_kp=None, contacts=None, pec_zone_counts=(0, 0, 0),
...
def main():
    ...
    pec_zone_counts = [0, 0, 0]               # interactions counted per PEC zone (index 0->Z1 ...)
        ...
        for frame_idx, frame in enumerate(generator):
            if frame_idx % SKIP != 0:
                draw_frame(frame, last_persons, entry_count, len(seller_interactions),
                           ...
                           contacts=last_contacts, pec_zone_counts=pec_zone_counts,
                           client_zone_counts=client_zone_counts)
            ...
            for b, r, tid in persons:
                ...
                if close:
                    ...
                    if contact_secs[tid] >= INTERACTION_MIN_SECS:
                        ...
                        pz = pec_zone_of_point(cc[0], cc[1])    # attribute to the served client's zone
                        if pz:
                            pec_zone_counts[pz - 1] += 1
                        print(f"[PEC] f{processed_frame_idx} INTERACTION +1 (client tid {tid} "
                              f"at t={timestamp:.1f}s) zone={pz or '-'} "
                              f"-> total={len(seller_interactions)} Z{pec_zone_counts}")
            ...
            draw_frame(frame, persons, entry_count, len(seller_interactions),
                       ...
                       contacts=contact_now, pec_zone_counts=pec_zone_counts,
                       client_zone_counts=client_zone_counts)
            sink.write_frame(frame)
